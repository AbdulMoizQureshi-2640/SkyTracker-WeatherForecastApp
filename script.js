    ...
    if (message.toLowerCase().includes("weather")) {
        ...
        if (city) {
            ...
            fetchWeatherDataFromAPI(city)
                ...
                .catch(error => {
                    // Handle API errors
                    addMessageToChatHistory("WeatherBot: Unable to fetch weather data. " + error.message);
                });
        } else {
        ...
        fetchGeminiResponse(message)
            ...
            .catch(error => {
                // Handle API errors
                addMessageToChatHistory("WeatherBot: Sorry, I couldn't process your request. " + error.message);
            });
    }
}
...
function fetchGeminiResponse(message) {
    return fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${geminiKey}`, {
        method: 'POST',
        headers: {
