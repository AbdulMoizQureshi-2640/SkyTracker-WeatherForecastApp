...
const apiKey = '041f43995ffd1beb6ab8390b40e9127f'; // OpenWeather API key
const geminiKey = 'AIzaSyD_zdEZq2Fdo-VSNY16jNrhVVlKxZj99Rs' //gemini API key
...
async function fetchWeatherData() {
    ...
    try {
        ...
        populateTable(forecastData); // Populate the forecast table with data
...
function processForecastData(data) {
    ...
    data.list.forEach(item => {
        ...
        if (days.length <= 5) {
            ...
            weatherConditions[item.weather[0].main] = (weatherConditions[item.weather[0].main] || 0) + 1;
        }
    });
...
function populateTable(forecastData) {
    const tbody = document.getElementById('table-body'); //GETTING TABLE BODY CATCHER
    ...
    forecastData.list.forEach(item => {
        ...
        // Check if the date has already been added to the uniqueDates
        if (!uniqueDates[date]) {
            uniqueDates[date] = {
                temp: Math.round(item.main.temp),
                weatherCondition: item.weather[0].description,
                weatherIcon: item.weather[0].icon || 'default_icon', // Use a default icon if missing
    ...
    // Convert the uniqueDates object to an array
    const uniqueEntries = Object.entries(uniqueDates).slice(0, 5);

    uniqueEntries.forEach(([date, data], index) => {
...
document.getElementById('filter').addEventListener('change', function () {
    ...
    if (filterValue === 'ascending') {
        ...
        const maxTemp = Math.max(...rows.map(row => parseInt(row.cells[1].textContent)));
...
async function sendMessageToChatbot(message) {
    ...
    if (message.toLowerCase().includes("weather")) {
        ...
        if (city) {
            ...
            addMessageToChatHistory("WeatherBot: " + formatWeatherResponse(weatherData));
        } else {
...
async function fetchWeatherDataFromAPI(city) {
    try {
...
async function fetchGeminiResponse(message) {
    try {
        ...
        return data; // Adjust this based on actual response structure
    } catch (error) {
